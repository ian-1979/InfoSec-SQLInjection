...
def check_table():
	inUser = userVar.get()
	inPass = passVar.get()
	# no longer vulnerable to "OR 1=1; -- in password field
	for row in cr.execute("Select * FROM users"):
		uname = row[0]
		passw = row[1]
		if (userVar.get() == uname and passVar.get() == passw):
			return True
	return False
...
def register_info():
	if check_valid() == True:
		if check_table() == False:
			inUser = userVar.get()
			inPass = passVar.get()
			# not vulnerable to hacker', 'none'); DELETE FROM users; --
			# not vulnerable to hacker', 'none'); UPDATE users SET pass = 'mine_now' WHERE user = 'admin'; --
			cr.execute(f"INSERT INTO users VALUES ('{inUser}', '{inPass}')")
			db.commit()
